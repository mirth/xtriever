...
# Features 002, 003 and 004 use the same device: `xtriever-lexical`'s scaffold module returns
# `Error::backend(NotImplemented(..))` from every method until the real modules replace it (T060).
status=0
for crate in xtriever-ffi xtriever-lexical xtriever-eval xtriever-dense; do
...
printf 'check-no-stubs: PASS — no scaffolding left in crates/{xtriever-ffi,xtriever-lexical,xtriever-eval,xtriever-dense}/src/\n'
