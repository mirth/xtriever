...
PR A was planned under Rule 3's ~800 changed lines and did not stay there: by its seventh
review round it carries about 1,550 inserted lines of Rust (the format change, the eval
harness's float sidecar, the reference generators' consolidation), which the owner waived for
this pull request; the reviewer's three-way split is recorded in `pr-description-a.md`. The
artefact rebuild in PR B is machine time, not diff.
...
- [X] T015e [US1] Review round 5 (PR A), six of seven findings applied (the 800-line budget, finding 7, stays waived by the owner): the checker refuses a mutation or oracle step it does not model and handles `replace`; the hybrid baseline's row lookup returns a clean error again; `export-vectors` reads only the sampled rows, by seek; the checker writes the generators' bytes (`sort_keys`); the cache-key test's baseline is version 3 with 2 as the miss; the Rust suites use the crate's own quantiser through `#[doc(hidden)] pub mod quantise` and no longer claim an independent restatement — that is `reference/dense_format3.py`, which mints every golden they replay
- [X] T015f [US1] Review round 6 (PR A), seven findings, all applied: the hybrid baseline and `export-vectors` no longer open the cache's index for two numbers the key, the embedder and the sidecar already give; the persistence test uses `support::assert_recovered`; the sidecar sample opens the file once and the byte layout is decoded and encoded in one place; ADR-0013's two remaining pointers to deleted files carry the superseded note; the direction property asserts the bound the half-step implies, `(1 − t) / (1 + t)`, instead of a statistic of random vectors; a missing golden fails the checker; the stored norm is checked only under Cosine, the metric that reads it, so a damaged norm under Dot or Euclidean degrades to the same scores rather than refusing the index
- [X] T015g [US1] Review round 7 (PR A), seven findings, all applied: one sidecar reader with one completeness check (the exact length), so an empty or wrong-width file is refused rather than derived from, and the embedder's width lives in the cache key; the stored norm is computed once at `add` and carried in the pending set (`StagedRow`); the persistence test again asserts an absent id and a reopened handle's binary-search recovery; T016 names the checker's real invocations; the exported quantiser asserts finiteness in debug builds and says callers validate first; the task list no longer claims a line budget the branch does not meet
- [X] T016 [US1] PR A gate: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets`, `cargo nextest run --workspace`, `cargo deny check`, the three cross-target checks, `cargo bench -p xtriever-dense --bench scan`, `python3 reference/gen_026_fixtures.py && python3 reference/gen_026_fixtures.py --check-oracle`; write `specs/026-eight-bit-precision/pr-description-a.md`. **⛔ Checkpoint C1 — the owner commits**, pushes, opens and merges PR A — **done**: PR A merged as #30
...
- [X] T023b [US2] The arithmetic (owner's decision 2026-09-21, after measuring): candle's eight-bit CPU kernel at 442 ms per embedding against 125 float; f16 expansion at load at 120 ms with the same SciFact quality to three decimals; the encoders construct their matmuls explicitly in f16 so candle's environment switches cannot change a vector, and `FINGERPRINT_Q8` / `MODEL_ID_Q8` name `compute=f16`; research D5, ADR-0015 and the contract record the measurement and the choice; `crates/xtriever-dense/examples/embed_timing.rs` is the record's tool (it measured the modes while candle's switches still governed the encoder; with the mode fixed in code it times the production path, and its header says so)

- [X] T023d [US2] The arithmetic, again (owner's decision 2026-09-22, after T029 on the emulator): f16 held on the host but not across platforms — the Android emulator disagreed with macOS-minted goldens on 36 of 800 hits and by 0.0134 in a re-rank score, where the float models had agreed on all 800 — and f32 restored the float models' parity (800 of 800, dense identical to the bit, re-rank within 6.7e-6) for 39 MB more resident memory across the two models; the encoders expand to `f32` (`QMatMul::Tensor`), the fingerprints say `compute=f32`, and every golden, cache, corpus and record minted under f16 is regenerated; ADR-0015, research D5 and the contract record both decisions

- [X] T023c [US2] Review PR B round 1, ten findings: nine applied (the packager's `local_dir` lookup guarded and `manifest.json` given one; the Python demo's input tests on the eight-bit defaults; every fixture consumer on the eight-bit defaults and the fixture re-minted; the re-ranker reads its GGUF once — one var-builder for the encoder and the classifier — and both loaders assert the header's layer-norm epsilon against the pinned `config.json` instead of falling back; the eight-bit embedder's fetch borrows only `config.json` and `tokenizer.json`, not the float weights; the encoder and the header reader are byte-identical across the two stage crates and `tests/twins.rs` fails if they drift; the re-ranker's `load` doc describes the two-artefact contract; the f16 expansion's own rounding is stated in the module docs, ADR-0015 and research D5); the tenth, Rule 3's budget, stays the owner's
...
- [X] T025 [US3] Rebuild the Wikipedia corpus with `cargo run --release -p xtriever-cli -- wiki build --out target/xt-wiki` (hours) and its goldens with `wiki expected`, replacing the format-2 artefact — **done** under `compute=f32`: 427,947 passages, 585,130,234 bytes, 745.9 min; verify PASS
- [X] T026 [P] [US3] Rebuild the 40-document fixture index and its goldens (`cargo run --release -p xtriever-ffi --example fixture_index -- swift/Xtriever/Tests/Fixtures`, the step `build-ios-package.sh --with-fixtures` runs), which every platform's parity test depends on, and point every consumer of the fixture — the Python suites' conftests, the FFI and pipeline test supports, `model_roundtrip.rs` — at the eight-bit directories it is minted with (review PR B round 1, finding 3: landed together with the minter's default so the tree is consistent at every commit). Done before T024's three-dataset gate; if that gate fails and the models revert, this re-mint is seconds to redo
- [X] T027 [P] [US3] Rebuild the demo corpus slices with `apps/python-wiki-demo/.venv/bin/wikidemo build --limit 2000 --out target/xt-wiki-slice-py` and the Rust slice, and regenerate the Android demo's measurement goldens — **done**: the 2,000-article slice and its goldens re-minted under `compute=f32`, staged by `build-android-package.sh`
...
- [X] T028 [US3] Record the measurement in `specs/026-eight-bit-precision/runs/`: `wikidemo measure` over the rebuilt corpus — parity against the new goldens, and the peak resident size against the 600 MB ceiling and the previous record (SC-005, FR-011) — **done**: `runs/measure-MacBookPro18,3-20260923T014434Z-mmap-threadsdefault-idle.json`, parity 800/800; peak resident size 639.4 MB, **over** the ceiling as SC-005 is worded (`underCeiling: false`), and peak `phys_footprint` 440.9 MB, under it (`footprintUnderCeiling: true`) — the measure ADR-0010 defines the ceiling on, now recorded beside resident size; SC-005's wording is the owner's to reconcile. The `f16` record kept as `…-compute-f16.json`
- [X] T029 [P] [US3] Re-run each demonstration's own checks against the rebuilt artefacts: `apps/python-wiki-demo/tests`, `android/xtriever` and `apps/android-wiki-demo` on the emulator, and `apps/ios-wiki-demo` on the simulator — **done except iOS**: `apps/python-wiki-demo/tests` 94 passed (the chonky tests run), `android/xtriever` 6/6 and the demo's measurement 800/800 on the emulator (`runs/android-….json`); iOS omitted by the owner (no device)

---

## Phase 6: Polish & Cross-Cutting Concerns

- [X] T030 [P] Update the format documentation in `crates/xtriever-dense/src/lib.rs` and the artefact tree in `specs/008-wiki-corpus/contracts/artefact.md` for format 3 — **done**
- [X] T031 [P] Update `android/xtriever/README.md`, `apps/python-wiki-demo/README.md` and `apps/ios-wiki-demo/README.md` where they name the index size, the model sizes or the format version — **done** (and `swift/Xtriever/README.md`, `apps/android-wiki-demo/README.md`, both packagers' size notes)
- [X] T032 Write `specs/026-eight-bit-precision/report.md`: the verdict, both pull requests, the evaluation table for all three datasets, the footprint before and after, the benchmark, and "deliberately not done" — no rescoring pass, no four-bit, no accelerator, no arbitrary user-supplied models (the next feature) — **done**
