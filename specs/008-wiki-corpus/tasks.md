...
- [ ] T042 [P] BEIR re-run (quickstart Step 6, Rule 5): `hybrid-rerank-v1` on SciFact, NFCorpus, FiQA with the cached embeddings and the committed indexes; `diff` the metrics against `specs/006-rerank-stage/baselines/*.json` — all three empty. ⛔ Any delta is a report (the read-only directory and `merge` touch no scoring; a delta means they do)
