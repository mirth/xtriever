...
- [ ] T019 [US1] In `crates/xtriever-pipeline/src/index.rs` (and `SparseEncoder::write_query_side` in the dense crate): `create_sparse` refuses a user `_sparse` field, appends `_sparse` to the schema (text, `standard`, indexed, not stored, the option's boost), verifies and copies the encoder's `tokenizer.json` and `idf.json` into `<dir>/sparse/` (the latter as `query-table.json`) and records their hashes; `open*` verifies them and builds the `SparseQuery`; `set_sparse_encoder`, `sparse()`, a count of truncated documents; `add` encodes each document's dense passage and writes `field_text` into `_sparse`, refusing without an encoder; `add_embedded` refuses on a sparse index; `add_encoded` takes caller-supplied vectors and expansions
