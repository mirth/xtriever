...
| V | Small, Explicit Interfaces | The `xtriever-core` traits (`Analyzer`, `LexicalIndex`, `Embedder`, `VectorIndex`, `Reranker`, `Ranker`), the on-disk format, and error semantics are unchanged — or the change has human review plus an ADR in `docs/adr/`. Core APIs are synchronous; async wrappers only in `xtriever-ffi` or server crates. Internal `DocId` is a dense `u32` and backends never see external string ids. One crate per stage, dependencies pointing downward only: `core` ← stage crates ← `pipeline` ← `ffi`/`cli`. | **PASS** (ADR-0016 accepted 2026-09-23) | No core trait changes and the dependency direction holds (the pipeline already depends on the dense crate). The on-disk format changes for sparse indexes only (descriptor version 3, the `sparse` record, the reserved `_sparse` field and the `sparse/` directory), with human review and ADR-0016, which the owner accepted after PR B merged. |
...
ADR-0016 is accepted; nothing else blocks. **Cleared** 2026-09-23: the owner accepted
ADR-0016 after PR B merged.

**Post-design gate (post-Phase 1)**: **FAIL, unchanged and expected** — **cleared** 2026-09-23
with the initial gate, when the owner accepted ADR-0016 (the only FAIL row, Principle V) — 2026-09-23, after
