...
- [X] T028 [US3] Make the error cases ergonomic in Swift if the generated enum needs it (in `swift/Xtriever/Sources/Xtriever/XtrieverIndex.swift`): a `public extension XtrieverError { var message: String }` returning each case's message/field text so tests and the demo can show it; no re-mapping — the generated cases are the API
- [X] T029 [US3] Run `xcodebuild test … -only-testing:XtrieverTests/ErrorTests` — **6 / 6 green**; then the whole simulator suite (`-skip-testing:XtrieverTests/DeviceMeasurementTests`) green. **Checkpoint — PR 3.**
