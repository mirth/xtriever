...
- [X] T042 **(passed on the third push — see report.md "CI" for the two failures before it)** Verify the CI job on a throwaway branch: one commit touching `crates/xtriever-lexical/README.md` (or a comment) must run `eval-smoke` and pass, a second push must hit the cache (no download in the log), and a commit touching only `docs/` must skip the job (US4 scenario 4); record the run URLs in `report.md`
