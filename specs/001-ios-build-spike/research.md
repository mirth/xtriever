...
| R4 | The constitution says ties break by ascending `DocId`; tantivy breaks them by ascending `DocAddress`. Identical at one segment, divergent beyond. | CITED | **Resolved by [ADR-0005](../../docs/adr/0005-tie-breaking-contract.md)** (accepted 2026-09-11): stages re-sort ties into ascending `DocId`. Not triggered by this spike (D5), but blocks the first multi-segment index. |
