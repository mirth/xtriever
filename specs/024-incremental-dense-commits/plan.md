...
| 2 | Don't touch the contract uninvited | No `xtriever-core` trait or `deny.toml` change. | PASS | Neither touched; compaction is inherent on `FlatIndex`, which the pipeline holds concretely. |
