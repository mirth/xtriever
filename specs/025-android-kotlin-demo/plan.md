...
committed 40-document fixture goldens on an emulator by the FR-004 rule — identifiers, order,
lexical and fused bits exact, model scores within 1e-3. **PR B** is the demo
...
ceiling is reported beside the figure for comparison only. Correctness is the hard claim, by
the FR-004 rule: hit identifiers, their order, the lexical bits and the fused bits identical to
the host's, and the two model-computed scores within 1e-3 (measured at 1.3e-7 and 3.3e-6,
because the matrix kernels compiled for this target round their reductions differently).
...
| II | Executable Oracles Over Prose (NON-NEGOTIABLE) | Acceptance tests are written and committed failing before implementation. Behaviour with a reference implementation is pinned to golden fixtures from `reference/` with tolerances stated in the spec. Ranking-affecting work reports nDCG@10 / Recall@100 deltas from `xtriever-eval` on SciFact / NFCorpus / FiQA. Invariants (analyzer determinism, index round-trips, filter algebra) are property-tested. | PASS | The acceptance test is the committed `swift/Xtriever/Tests/Fixtures/expected.json`, replayed on the emulator at the depth each golden carries (plus depth 0 against the no-re-ranker goldens) and committed failing first (Rule 4); the tolerance split it applies is stated in FR-004 and in the test itself. Ranking is untouched, so there are no nDCG/Recall deltas to report — FR-014 forbids a ranking change and the gate proves it by the absence of a diff in the stage crates. |
