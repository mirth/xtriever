...
- [X] T001 Add `[profile.android]` to `Cargo.toml` — `inherits = "release"`, `strip = "debuginfo"` — with a comment stating why, beside the existing `[profile.wheel]`: the binding metadata lives in the symbol table and `strip = true` removes it (research D4; measured 0 `UNIFFI_META` symbols stripped against 30 unstripped)
...
- [X] T003 [P] Create `gradle/libs.versions.toml` pinning the Android Gradle Plugin, Kotlin, Compose and Java Native Access versions the generated project resolved, each with the date it was resolved
- [X] T004 [P] Extend `.gitignore`: the generated Kotlin bindings, `android/xtriever/src/main/jniLibs/`, `apps/android-wiki-demo/src/main/assets/`, every Gradle `build/` directory, `local.properties`, `.gradle/` — nothing generated is committed (FR-003)
...
- [X] T009 [US1] Write `android/xtriever/build.gradle.kts`: library plugin, minimum API 26, `ndk { abiFilters += "arm64-v8a" }`, the Java Native Access Android archive dependency from the version catalogue, the generated Kotlin as a source set, and the instrumentation runner
- [X] T010 [P] [US1] Write `android/xtriever/src/main/kotlin/.../DeviceSupport.kt`: read `/proc/cpuinfo`, require `fphp` and `asimdhp`, and expose a single result that is either supported or a refusal naming the requirement — consulted before any native call, because an unsupported processor dies on an illegal instruction that no `catch` can reach (research D5, FR-002)
- [X] T011 [US1] Write `android/xtriever/src/main/kotlin/.../XtrieverIndex.kt`, the wrapper mirroring `swift/Xtriever/Sources/Xtriever/XtrieverIndex.swift`: open with a load path, hold the paths, the open and model-load times and the engine's index information, close deterministically, and add no capability the Swift wrapper lacks (FR-001)
- [X] T012 [US1] Teach `scripts/build-android-package.sh`'s `--with-fixtures` to build and stage the 40-document fixture index and its goldens for the instrumented test, as the iOS script does for the simulator
