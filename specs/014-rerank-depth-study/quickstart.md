...
```bash
reference/.venv-012/bin/python -m pytest reference/tests_014 -q      # ImportError: no module rerank_study
```
...
for d in scifact nfcorpus fiqa; do
  ...
  reference/.venv-012/bin/python reference/gen_003_fixtures.py --verify-run $S/$d/e2e-d50.jsonl --qrels reference/datasets/beir/$d/qrels/test.tsv --report specs/014-rerank-depth-study/runs/e2e-d50.$d.json
done
...
```bash
for d in scifact nfcorpus fiqa; do
  reference/.venv-012/bin/python reference/rerank_study.py all --dataset $d --explain $S/$d/explain-d50.jsonl \
...
reference/.venv-012/bin/python reference/rerank_study.py table && reference/.venv-012/bin/python reference/rerank_study.py decide
```
