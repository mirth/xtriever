...
- Hand-written `unsafe` only in `xtriever-dense`, and there only in SIMD kernels and in
  read-only memory mapping behind a non-default feature; each block preceded by `// SAFETY:` and
  tested against the safe path (ADR-0007). `xtriever-ffi` may additionally carry a crate-level
...
**Version**: 1.2.0 | **Ratified**: 2026-09-10 | **Last Amended**: 2026-09-12
