...
- Hand-written `unsafe` only in `xtriever-dense` SIMD kernels, each block preceded by
  `// SAFETY:` and tested against the safe path. `xtriever-ffi` may additionally carry a
  crate-level `#![allow(unsafe_code)]` covering FFI scaffolding generated by `uniffi`; every
  module holding hand-written logic MUST re-declare `#![deny(unsafe_code)]` (ADR-0003).
