...
//! reopens.
//!
//! The generator is a fixed-seed LCG (`support::Lcg`): deterministic without a dependency. The
//! `mint` test writes the *steps* and the crate's provisional hits; the committed expectations
//! are then written by `reference/gen_026_fixtures.py --write-oracle`, which recomputes every
//! one in Python from the contract's arithmetic — so the golden is generated by `reference/`
//! (Principle II), `replay` checks this crate against it, and CI runs `--check-oracle` so the
...
/// Mint `tests/support/v3_oracle.json`'s steps from the implementation this test is compiled
/// against, then run `reference/gen_026_fixtures.py --write-oracle` so the committed
/// expectations are the reference's. Run once per format change, and only with the owner's
/// decision recorded — re-minting is how an oracle stops being one, so it belongs to a feature
/// that says why (here, ADR-0015).
#[test]
#[ignore]
fn mint() {
