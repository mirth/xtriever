...
use xtriever_eval::run::{
    HybridConfig, RerankConfig, SparseCacheKey, SparseSettings, read_sparse_weights,
    write_sparse_weights,
};
...
/// The weights round-trip bit for bit, empty expansions included; a file with another count,
/// or cut short, or with bytes after the last document, is refused.
#[test]
fn the_weights_round_trip_and_refuse_a_damaged_file() {
    ...
    let expansions = vec![
        ...
        vec![],
        vec![(999, f32::from_bits(0x3f80_0001))],
    ];
